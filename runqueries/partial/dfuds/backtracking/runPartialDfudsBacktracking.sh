...
for type_fun in 0 1; do
  ...
  time_csv="../../../outputs/partial/dfuds/backtracking/results-f$type_fun-time-opt.csv"
  nodes_csv="../../../outputs/partial/dfuds/backtracking/results-f$type_fun-nodes-opt.csv"
  ...
  for k in 1 10 100 1000; do
    ...
    for file in j3 j4 p2 p3 p4 s1 s2 s3 s4 t2 t3 t4 ti2 ti3 ti4 tr1 tr2; do
      ...
      results_file="../../../outputs/partial/dfuds/backtracking/$file-f$type_fun-k$k-results-opt.txt"
      chmod +x "$output_file"
      > "$results_file"
